...
public class ConnectionDAO extends SQLiteOpenHelper {
  ...
  private static final String SQL_CREATE_TABLE = "CREATE TABLE " + Connection.TABLE_NAME + " (" + Connection._ID
      + " INTEGER PRIMARY KEY," + Connection.COLUMN_NAME_TYPE + " INTEGER NOT NULL," + Connection.COLUMN_NAME_TITLE
      + " TEXT NOT NULL," + Connection.COLUMN_NAME_DESCRIPTION + " TEXT," + Connection.COLUMN_NAME_IMAGE + " TEXT,"
  ...
  public void onCreate(SQLiteDatabase db) {
    ...
    connection.setTitle("Radiofabrik Salzburg");
    connection.setDescription("Addicted²Random at radiofabrik.at");

    try {
      connection.setUri(new URI("ws://master.radiofabrik.at:8080"));
    } catch (URISyntaxException e) {
    }

    ContentValues values = Connection.toContentValues(connection);
    db.insert(Connection.TABLE_NAME, null, values);
  }
  ...
  public boolean delete(Connection connection) {
    if(connection.getId() == null) return false;
    
    SQLiteDatabase db = getWritableDatabase();

    String[] args = new String[] { String.valueOf(connection.getId()) };
    int count = db.delete(Connection.TABLE_NAME, SELECT_BY_ID, args);
